...
def pack_object_at(cursor, offset, as_stream):
    ...
    if as_stream:
        stream = DecompressMemMapReader(
            buffer(data, total_rela_offset), False, uncomp_size)
...
class IndexWriter(object):
    ...
    **Note:** currently only writes v2 indices"""
    ...
    def write(self, pack_sha, write):
        ...
        # fanout
        # fanout or list with 64 bit offsets
        tmplist = list((0,) * 256)
        for t in self._objs:
            tmplist[ord(t[0][0])] += 1
        # END prepare fanout
...
class PackIndexFile(LazyMixin):
    ...
    def _offset_v2(self, i):
        ...
        offset = unpack_from(
            ">L", self._cursor.map(), self._pack_offset + i * 4)[0]
        ...
        if offset & 0x80000000:
            offset = unpack_from(
                ">Q", self._cursor.map(), self._pack_64_offset + (offset & ~0x80000000) * 8)[0]
...
class PackFile(LazyMixin):
    ...
    **Note:** at some point, this might be implemented using streams as well, or
    ...
    def data(self):
        ...
        # can use map as we are starting at offset 0. Otherwise we would have
        # to use buffer()
        return self._cursor.use_region().map()

    def checksum(self):
        """:return: 20 byte sha1 hash on all object sha's contained in this file"""
        return self._cursor.use_region(
            self._cursor.file_size() - 20).buffer()[:]
    ...
    def info(self, offset):
        ...
        return pack_object_at(
            self._cursor, offset or self.first_object_offset, False)[1]
    ...
    def stream(self, offset):
        ...
        return pack_object_at(
            self._cursor, offset or self.first_object_offset, True)[1]
...
class PackEntity(LazyMixin):
    ...
    __slots__ = ('_index',           # our index file
                 '_pack',            # our pack file
                 # on demand dict mapping one offset to the next consecutive
                 # one
                 '_offset_map'
                 )
        ...
        if use_crc:
            ...
            while cur_pos < next_offset:
                ...
                this_crc_value = crc_update(
                    buffer(pack_data, cur_pos, size), this_crc_value)
                cur_pos += size
    ...
    def collect_streams_at_offset(self, offset):
        ...
        # a REF delta, or a base object, as OFFSET deltas are resolved by the
        # pack
    ...
    def write_pack(cls, object_iter, pack_write, index_write=None,
        ...
        **Note:** writes only undeltified objects"""
        ...
        # current offset into the pack file
        ofs = 0
        index = None
        wants_index = index_write is not None

        # write header
        pwrite(pack('>LLL', PackFile.pack_signature,
    ...
    @classmethod
    def create(cls, object_iter, base_dir, object_count=None,
