...
class TestPack(TestBase):
    ...
    def test_pack_index(self):
        ...
        for indexfile, version, size in (
                self.packindexfile_v1, self.packindexfile_v2):
    ...
    def test_pack(self):
        ...
        for packfile, version, size in (
                self.packfile_v2_3_ascii, self.packfile_v2_1, self.packfile_v2_2):
    ...
    def test_pack_entity(self, rw_dir):
        ...
        for ppath, ipath, num_obj in zip(
                (pack_path, ) * 2, (index_path, None), (len(pack_objs), None)):
            pfile = open(ppath, 'wb')
            iwrite = None
            if ipath:
                ifile = open(ipath, 'wb')
            ...
            # make sure we rewind the streams ... we work on the same objects
            # over and over again
            if iteration > 0:
                rewind_streams()
            # END rewind streams
