...
def delta_chunk_apply(dc, bbuf, write):
    ...
    if dc.data is None:
        ...
        # Considering data can be larger than 127 bytes now, it should be worth
        # it
...
class DeltaChunk(object):
    ...
    __slots__ = (
        'to',       # start offset in the target buffer in bytes
                    # size of this chunk in the target buffer in bytes
                    'ts',
                    # start offset in the source buffer in bytes or None
                    'so',
                    # chunk of bytes to be added to the target buffer,
                    'data',
...
class DeltaChunkList(list):
    ...
    def compress(self):
        ...
        while i < slen:
            ...
            i += 1
            if dc.data is None:
                if first_data_index is not None and i - \
                        2 - first_data_index > 1:
                    # if first_data_index is not None:
                    nd = StringIO()                     # new data
                    # start offset in target buffer
        ...
        # print "INFO: Reduced delta list len to %f %% of former size" %
        # ((float(len(self)) / slen_orig) * 100)
        return self
    ...
    def check_integrity(self, target_size=-1):
        ...
        if target_size > -1:
            ...
            assert reduce(
                lambda x, y: x + y, (d.ts for d in self), 0) == target_size
...
class TopdownDeltaChunkList(DeltaChunkList):
    ...
    def connect_with_next_base(self, bdcl):
        ...
        while dci < slen:
            ...
            # all add-chunks which are already topmost don't need additional
            # processing
