...
class DecompressMemMapReader(LazyMixin):
    ...
    **Note:** The chunk-size should be carefully selected as it will involve quite a bit
    ...
    def __init__(self, m, close_on_deletion, size=None):
        ...
        if size is not None:
            # size of uncompressed data to read in total
            self._s = size
        self._br = 0                            # num uncompressed bytes read
        # start byte of compression window
        self._cws = 0
        # end byte of compression window
        self._cwe = 0
        # number of compressed bytes read
        self._cbr = 0
        # is True if we parsed the header info
        self._phi = False
        # close the memmap on deletion ?
    ...
    def read(self, size=-1):
        ...
        # if window is too small, make it larger so zip can decompress
        # something
        ...
        if dcompdat and (
                len(dcompdat) - len(dat)) < size and self._br < self._s:
...
class DeltaApplyReader(LazyMixin):
    ...
    * A list of one byte commands (cmd) which are followed by a specific protocol:
    ...
    __slots__ = (
        ...
        # memory map of the delta-applied data
        "_mm_target",
        # actual number of bytes in _mm_target
        "_size",
    ...
    def __init__(self, stream_list):
        ...
        assert len(
            stream_list) > 1, "Need at least one delta and one base stream"
    ...
    def _set_cache_too_slow_without_c(self, attr):
        ...
        # print "Handling %i delta streams, sizes: %s" % (len(self._dstreams),
        # [ds.size for ds in self._dstreams])
    ...
    def _set_cache_brute_(self, attr):
        ...
        # assure that all buffers we use are large enough to hold all the
        # results
        if len(self._dstreams) > 1:
        ...
        for (dbuf, offset, src_size, target_size), dstream in reversed(
            ...
            ddata.write(dbuf)
            # read the rest from the stream. The size we give is larger than
            # necessary
            stream_copy(
